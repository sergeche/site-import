...
function DestStream(dest, options) {
	...
	var outFolder = function(file) {
		// Dynamically generate `out` folder depending in file prefix.
		// XXX Actually, this is not the right solution at all in VinylFS:
		// changing path (adding or removing folder) of vinyl file will cause
		// in broken symlink because VinylFS resolves original file source
		// based on current `file.path`, not first entry of `file.history`
		return file.prefix ? path.join(dest, file.prefix) : dest;
	};

	this.files = vfs.dest(outFolder, options).once('end', onEnd);
	this.symlinks = vfs.symlink(outFolder, options).once('end', onEnd);
}
